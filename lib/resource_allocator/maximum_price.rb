...
module MaximumPrice
  ...
  def self.get_price_hours_cost(region, hours, price)
    ...
    call_flag = 1
    converted_east_cost = Hash[$region[:'us-east'].map { |k, v| [k, (v * 10000).to_i] }]
    converted_west_cost = Hash[$region[:'us-west'].map { |k, v| [k, (v * 10000).to_i] }]
    converted_asia_cost = Hash[$region[:'asia'].map { |k, v| [k, (v * 10000).to_i] }]

    price = ((price/hours.to_f)*10000)
    
    east_server = OptimizeCPU.optimize_servers(converted_east_cost.values, price.to_i, $region[:'us-east'].stringify_keys.invert, call_flag)
    west_server = OptimizeCPU.optimize_servers(converted_west_cost.values, price.to_i, $region[:'us-west'].stringify_keys.invert, call_flag)
    asia_server = OptimizeCPU.optimize_servers(converted_asia_cost.values, price.to_i, $region[:'asia'].stringify_keys.invert, call_flag)
    ...
    if east_server.present?
      east_hash = {
        "region": "us-east",
        "total_cost": "$" + (((price.to_f)*8)/10000).to_s,
        "servers": east_server
      }
    end
